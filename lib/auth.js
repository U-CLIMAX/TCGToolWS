...
export const handleLogin = async (c) => {
  ...
  try {
    ...
    const user = await db
      .prepare('SELECT id, hashed_password, salt, email, role FROM users WHERE email = ?1')
      .bind(email)
      .first()
    if (!user) {
...
export const handleAfdianWebhook = async (c) => {
  ...
  if (!sign) {
    console.warn('Afdian Webhook: No signature provided.')
    return c.json({ ec: 403, em: 'No signature' })
  }

  // 2. 驗證簽名 (正式環境用)
  const isVerified = await verifyAfdianSignature(c.env.AFDIAN_PUBLIC_KEY, payload, sign)
