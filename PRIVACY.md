...
<p align="center">
  <a href="#隐私政策"><b>简体中文</b></a> &nbsp;|&nbsp; <a href="#privacy-policy"><b>English</b></a>
</p>
...
本专案秉持 **最小化收集** 原则，仅在实现具体功能所必需的前提下收集最少的信息。
...
| **用户生成内容**   | 个人私有卡组（卡片 ID、卡组名称、系列识别码、游戏模式、封面卡、标签分类、编辑撤销历史）                      | Cloudflare D1 边缘数据库 (`decks`)                                    | 归属于用户账号，随用户账号注销连带级联删除                                                                                                                  |
...
4. **服务安全与合规保障**：通过分布式速率限制抵御针对登录接口和验证码接口的恶意凭证撞库、DoS/DDoS 拒绝服务攻击。
...
2. **Brevo (原 Sendinblue)**：
   - **用途**：事务型邮件服务，用于发送用户注册时的 6 位验证码以及找回密码时的重置链接。
